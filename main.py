...
def train(epoch):
    ...
    for subIter in range(subsetN):
        ...
        print('Allocated:{}MB'.format(torch.cuda.memory_allocated()/1024/1024))
        print('Cached:{}MB'.format(torch.cuda.memory_cached()/1024/1024))
        ...
        for iteration, (query, positives, negatives, 
            ...
            # print("shape of vladQ: ", vladQ.shape)
            # print("shape of vladP: ", vladP.shape)
            # print("shape of vladN: ", vladN.shape)
...
def save_qFeat(eval_set, epoch=0):
    saved_qFeat_filename = 'result/qFeatures-dim-2048.npy'
    checkpoint_root = 'checkpoint/checkpoint.2022-07-14-10-20-35.pth.tar'

    test_data_loader = DataLoader(dataset=eval_set,
                                  num_workers=opt.threads, batch_size=opt.cacheBatchSize, shuffle=False,
                                  pin_memory=cuda)

    model.load_state_dict(torch.load(checkpoint_root)['state_dict'])
    ...
    np.save(saved_qFeat_filename, np.array(qFeat))
    ...
    with h5py.File(initcache, mode='w') as h5: 
        with torch.no_grad():
            ...
            for iteration, (input, indices) in enumerate(data_loader, 1):
                input = input.to(device)
                image_encoder = model.encoder(input)
                # 这里并没有用model的pooling层，这是为什么呢
                image_descriptors = image_encoder.view(input.size(0), encoder_dim, -1).permute(0, 2, 1)
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    filename = 'checkpoint.' + datetime.now().strftime('%Y-%m-%d-%H-%M-%S') + '.pth.tar'
...
if __name__ == "__main__":
    ...
    whole_save_set = dataset.get_whole_save_set()
    ...
    if opt.mode.lower() == 'test':
        ...
        save_qFeat(whole_save_set, epoch)
    elif opt.mode.lower() == 'cluster':
